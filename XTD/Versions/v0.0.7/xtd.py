...
if optParsed[2] == False:
	...
	# Relation will be concepted as a dictionary of dictionaries
	relationsDict = {};

	# Initialising the dictionary
	for tmpTN in d.items():
		relationsDict[tmpTN[0]] = {};
	...
	loopChange = True;
	# Looping through the tables multiple times
	while (loopChange):
		loopChange = False;



		# This is the part when checking relation kicks in
		for rTabName in d.items():
			tKeysA = relationsDict.keys();
			tKeysB = rTabName[0];

			#tColsA = d[rTabName[0]];
			#tColsB = d[rTabName]

			# tt contains name of the table in relation to
			# tKeysB contains the name of the table that has relation with something
			for tt in tKeysA:
				...
				# 2(a) -> a == b, then R(a,b) = 1:1
				if tt == tKeysB:
					print("it is the same key");
					relationsDict[tKeysB][tt] = '1:1';
					#loopChange = True;
				# 2(b) -> a != b && a -> b && a -> b, then R(a,b) = N:M
				else:
	...
	for tabName in d.items():
		...
		for tRel in relationsDict[tabName[0]]:
			if tRel == tabName[0]:
				outAcc += "<relation to=\""+tabName[0]+"\" relation_type=\""+relationsDict[tabName[0]][tRel]+"\" />\n";
